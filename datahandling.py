...
def plot_item_descriptives_table(scored: pd.DataFrame) -> None:
    ...
    table_data = [[r[k] for k in ["Item", "Factor", "Reversed"] +
                   [f"{t} mean" for t in TEMPS for _ in [1]] +
                   [f"{t} SD" for t in TEMPS for _ in [1]] +
                   [f"{t} n" for t in TEMPS for _ in [1]]]
                  for r in rows]

    # Reorder columns: interleave mean/SD/n per temp
